...
class LwF_LoRA(ContinualLearningMethod):
    ...
    def compute_loss(self, images, labels, text_tokenized):
        ...
        old_logits_prime:torch.Tensor = F.log_softmax(old_logits/self.config.train.distill_temp, dim = 1)
        ...
        logits_prime:torch.Tensor = F.log_softmax(logits / self.config.train.distill_temp, dim = 1)

        loss_kd = F.kl_div(logits_prime, old_logits_prime, reduction= "batchmean", log_target= True) * (self.config.train.distill_temp * self.config.train.distill_temp)

        #@ loss_ce
        loss_ce = self.criterion(logits, labels)

        loss = loss_ce + loss_kd * self.config.train.lambda_old
        return loss